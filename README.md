...
# allowJS 옵션에는 N으로 답하고 수동으로 tsconfig에 allowJS: true를 추가합니다.
# 나머지 질문에는 y를 답합니다.
```

그리고 vite 설정을 다음과 같이 업데이트 해줍니다.
...
아래처럼 결과를 확인할 수 있게 간단한 출력하나만 추가하고
어플리케이션을 실행시켜봅니다.
...
바로 가격표시에 대한 부분의 메시지를 작성하지 않았습니다. 왜그럴까요?
가격이나 이런 단위 체계의 국제화는 브라우저 api에 내장되어 있습니다.
이것을 이용해서 해당 부분을 수정해보겠습니다.
